...
const optimizeLineBreaks = (text) => {
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (rules.isSpace(word)) {
        if (buffer && rules.isOpenParen(buffer.slice(-1))) {
           } else {
          if (buffer) {
      ...
      if (rules.isOpenParen(word)) {
        shouldMerge = true;
      }

      else if (rules.isSpace(word) && buffer && rules.isOpenParen(buffer.slice(-2, -1))) {
        shouldMerge = true;
      }

      else if (rules.isSpace(nextWord)) {
        shouldMerge = false;
      }

      else if (rules.punctuation.has(nextWord)) {
        shouldMerge = true;
      }

      else if (rules.punctuation.has(word)) {
        shouldMerge = false;
      }
      else if (rules.startsWithSmallKana(nextWord)) {
        shouldMerge = true;
      }
      else if (rules.suffixes.has(nextWord) && !rules.isOpenParen(nextWord)) {
        shouldMerge = true;
      }
      ...
      else if (rules.isEnglish(word) && rules.isEnglish(nextWord)) {
        shouldMerge = true;
      }
